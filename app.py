...
# Load trained model + encoders (trained on Work_JL_DB_Cleaned.csv)
@st.cache_resource
def load_model():
...
uploaded_file = st.file_uploader(
    "📤 Upload your sheet (Create Date, JetLearn Deal Source, Country, Age, HubSpot Deal Score)",
    type="csv"
)
...
if uploaded_file is not None:
    ...
    if not all(col in df.columns for col in required_cols):
        st.error("❌ Your file must contain all of these columns: " + ", ".join(required_cols))
    else:
        ...
        st.subheader("📅 Monthly Predicted Enrollments (Aggregated)")
        st.dataframe(monthly_pred)

        st.markdown("---")
        st.subheader("📋 Row-wise Prediction (with probabilities)")
        st.dataframe(df[['Create Date', 'Country', 'JetLearn Deal Source', 'Age', 'HubSpot Deal Score', 'Enroll_Prob']])
